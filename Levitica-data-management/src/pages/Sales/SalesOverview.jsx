...
const STAGE_COLORS = ["bg-blue-400", "bg-blue-500", "bg-brand", "bg-violet-500", "bg-purple-500", "bg-amber-500"];
...
export default function SalesOverview() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      setLoading(true);
      try {
        const [dashRes, pipelineRes, sourceRes, activitiesRes, tasksRes] = await Promise.all([
        ...
        setUpcomingFollowups(tasks.slice(0, 6).map((t) => ({
  ...
  const activeStages = pipelineStages.filter((s) => s.stage && s.stage !== "won" && s.stage !== "lost");
  const maxPipeline = Math.max(...activeStages.map((s) => s.count || 0), 1);
                    ...
                    <div key={stage.stage || i} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="h-6 rounded-lg bg-gray-100 overflow-hidden">
                          <div className={`h-full rounded-lg ${STAGE_COLORS[i % STAGE_COLORS.length]}`} style={{ width: `${((stage.count || 0) / maxPipeline) * 100}%` }} />
                        </div>
                      </div>
                      <span className="text-sm font-semibold text-body tabular-nums w-6 text-right shrink-0">{stage.count ?? 0}</span>
                    </div>
                  ))}
                </div>
              </div>
                      ...
                      return (
                        <div key={i} className="px-5 py-3.5 flex items-start gap-3 hover:bg-gray-50/50 transition">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-sm text-body mt-0.5">{item.company} · {item.user} · {item.date}</p>
                          </div>
                        </div>
                      );
                    })
                  ...
                  {leadsBySource.length === 0 ? (
                    ...
                    leadsBySource.map((item, i) => (
                      <div key={item.source || i} className="flex items-center gap-3">
                        <span className="text-sm font-medium text-body w-24 shrink-0">{item.source || "Unknown"}</span>
                        <div className="flex-1 min-w-0">
                          <div className="h-6 rounded-lg bg-gray-100 overflow-hidden">
                            <div className={`h-full rounded-lg ${STAGE_COLORS[i % STAGE_COLORS.length]}`} style={{ width: `${((item.count || 0) / maxLeads) * 100}%` }} />
                          </div>
                        </div>
                        <span className="text-sm font-semibold text-body tabular-nums w-6 text-right shrink-0">{item.count ?? 0}</span>
                      </div>
                    ))
                  )}
                </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-gray-50">
                  {upcomingFollowups.length === 0 ? (
                    <div className="px-5 py-6 text-center text-body text-sm">No upcoming follow-ups.</div>
                  ) : (
                    upcomingFollowups.map((item, i) => (
                      <div key={i} className="px-5 py-3.5 flex items-start gap-3 hover:bg-gray-50/50 transition">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-brand-dark">{item.title}</p>
                          <p className="text-sm text-body mt-0.5">{item.date}{item.subtitle ? ` · ${item.subtitle}` : ""}</p>
                        </div>
                      </div>
                    ))
                  )}
